...
interface SpotifySavedTracksResponse {
  items: {
    track: { id: string; album: { id: string }; artists: { id: string }[] };
  }[];
}
...
export const analyzeAndStoreUserVibes = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const albumGenrePromises = savedTracks.map(async (item: SavedTrackItem) => {
      try {
        ...
        console.error(
          `Error fetching genres for album ${item.track.album.id}:`,
          err
        );
        return [];
      }
