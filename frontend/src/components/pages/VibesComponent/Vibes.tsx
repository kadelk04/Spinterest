...
export const Vibes = ({ expanded }: VibesProps) => {
  ...
  const { 
    userVibes, displayVibe, loading, calculating, error, 
    fetchUserVibes, analyzeAndStoreUserVibes, lastFetched, pickRandomVibe 
  } = useVibesAPI();

  const lastCalculatedDate = localStorage.getItem('lastCalculatedVibe');
  const isDisabled = lastCalculatedDate 
    ? new Date().getTime() - new Date(lastCalculatedDate).getTime() < 7 * 24 * 60 * 60 * 1000
    : false;
