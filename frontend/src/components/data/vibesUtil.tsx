...
export const useVibesAPI = () => {
  ...
  const fetchUserVibes = async () => {
    try {
      ...
      if (data.vibes && data.vibes.length > 0) {
        setDisplayVibe(data.vibes[Math.floor(Math.random() * data.vibes.length)]);
      } else {
  ...
  const pickRandomVibe = () => {
    ...
    let newVibe = displayVibe;
    while (newVibe === displayVibe) {
      const randomIndex = Math.floor(Math.random() * userVibes.length);
      newVibe = userVibes[randomIndex];
    }
    setDisplayVibe(newVibe);
  };
